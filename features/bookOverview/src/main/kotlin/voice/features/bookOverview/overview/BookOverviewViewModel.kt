...
class BookOverviewViewModel(
  ...
  internal fun state(): BookOverviewViewState {
    ...
    return BookOverviewViewState(
      ...
      miniPlayer = currentBookId?.let { id ->
        val currentBook = books.firstOrNull { it.id == id }
        currentBook?.let { book ->
          val overlaidBook = livePlaybackState.value?.let(book::overlay) ?: book
          overlaidBook.toMiniPlayerViewState(
  ...
  fun onBookClick(id: BookId) {
    navigator.goTo(Destination.BookDetails(id))
  }
