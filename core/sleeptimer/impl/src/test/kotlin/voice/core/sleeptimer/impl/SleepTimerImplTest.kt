...
class SleepTimerImplTest {
  ...
  @Test
  fun volumeIsNeverFadedDuringCountdown() = testScope.runTest {
    sleepTimer.enable(SleepTimerMode.TimedWithDuration(5.seconds))

    // run through the entire countdown, including the would-be fade-out window and the pause
    advanceTimeBy(6.seconds)
    yield()

    // volume stays at full the whole time; it is only ever (re)set to 1F, never lowered
